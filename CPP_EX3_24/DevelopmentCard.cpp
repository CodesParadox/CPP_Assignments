...
namespace ariel{
    DevelopmentCard* DevelopmentCardFactory::createCard(CardType type, Catan* game){
        ...
        {
        case Promotion:
            throw invalid_argument("Promotion card by itself cannot be created, please specify the type of promotion card you want.");
            break;
        case RoadBuilding:
    ...
    void RoadBuildingCard::use(){
        ...
        if (owner == nullptr){
            throw invalid_argument("Owner of the card is not set\n");
        }
    ...
    void KnightCard::use(){
        ...
        throw invalid_argument("Knight card cannot be used in this assignment\n");
    }

    void VictoryPointCard::use(){
        // no use in assignment
        throw invalid_argument("Victory Point card cannot be used in this assignment. A victory point is automatically added to the player upon buying\n");
    }

    // Overloading the << operator for each card type
    ostream& operator<<(ostream &out, const VictoryPointCard &card){
        return out << "Victory Point Card";
    }

    ostream& operator<<(ostream &out, const KnightCard &card){
        return out << "Knight Card";
    }

    ostream& operator<<(ostream &out, const MonopolyCard &card){
        return out << "Monopoly Card";
    }

    ostream& operator<<(ostream &out, const YearOfPlentyCard &card){
        return out << "Year of Plenty Card";
    }

    ostream& operator<<(ostream &out, const RoadBuildingCard &card){
        return out << "Road Building Card";
    }

    ostream& operator<<(ostream &out, const PromotionCard &card){
        return out << "Promotion Card";
    }

    ostream& operator<<(ostream &out, const DevelopmentCard &card){
        switch (card.type){
            case Promotion:
                return out << "Promotion Card";
                break;
