...
namespace ariel {
    ...
    int Board::placeSettlement(Player& owner, size_t index){
        ...
        if (indexToRowColPos(index, tileX, tileY, tilePos) == -1){
            throw invalid_argument("Failed to place settlement at " + to_string(index)
            + "\nInvalid index\n");
        }
                ...
                if (board[row][col]->hasIndexVertex(index)){
                    for (size_t pos = 0; pos < 6; pos++){
                        if (board[row][col]->getVertex((VertexPosition)pos)->getIndex() == index){
                            ...
                            {
                                throw invalid_argument("Failed to place settlement at " + to_string(tileY) + ", " + to_string(tileX) + " on " + to_string(tilePos)
    ...
    int Board::placeRoad(Player& owner, size_t index1, size_t index2){
        ...
        if (indexToRowColPosRoad(index1, index2, tileX, tileY, tilePos) == -1){
            throw invalid_argument("Failed to place road  at " + to_string(index1) + ", " + to_string(index2)
            + "\nInvalid index\n");
        }
        ...
        if (hasBuildingNear(owner, tileX, tileY, (EdgePosition)tilePos) == 0){
            throw invalid_argument("Failed to place road at Y,X " + to_string(tileY) + ", " + to_string(tileX) + " on " + to_string(tilePos)
    ...
    for (size_t row = 0; row < numberOfRows; ++row) {
        ...
        if (row == 0){
            cout << setw(35 - padding[row]) << " "; // Adjust leading spaces
        ...
        if (row < 3) {
            cout << setw(33 - padding[row]) << " "; // Adjust leading spaces
            for (size_t col = 0; col < boardSize[row]; ++col) {
                // if there is a road on this, color it with players color
                cout << *board[row][col]->getEdge(EdgePosition::EDGE_TOP_LEFT);
                cout << setw(5) << *board[row][col]->getEdge(EdgePosition::EDGE_TOP_RIGHT);
                if (col < boardSize[row]) {
                    cout << setw(7);
                }
            }
            ...
            cout << setw(28 - padding[row]) << " "; // Adjust leading spaces
            for (size_t col = 0; col < boardSize[row]; ++col) {
                cout << *board[row-1][col]->getEdge(EdgePosition::EDGE_BOTTOM_LEFT);
                cout << setw(5) << *board[row-1][col]->getEdge(EdgePosition::EDGE_BOTTOM_RIGHT);
                if (col < boardSize[row]) {
                    cout << setw(7);
                }
            }
            cout << *board[row-1][boardSize[row-1]-1]->getEdge(EdgePosition::EDGE_BOTTOM_LEFT)
                        << setw(5) << *board[row-1][boardSize[row-1]-1]->getEdge(EdgePosition::EDGE_BOTTOM_RIGHT);
            cout << endl;
        }
        

        // Print the middle vertices
        cout << setw(30 - padding[row]) << " "; // Adjust leading spaces
        for (size_t col = 0; col < boardSize[row]; ++col) {
        ...
        cout << *board[row][boardSize[row]-1]->getVertex(VertexPosition::VERTEX_TOP_RIGHT);
        cout << endl;
        ...
        for (size_t col = 0; col < boardSize[row]; ++col) {
            ...
            cout << setw(2) << " " << getResourceName(board[row][col]->getResource()) << "(" << board[row][col]->getNumber() << ")";
            if (col < boardSize[row]) {
                cout << setw(2) << " ";
            }
        }
        cout << *board[row][boardSize[row]-1]->getEdge(EdgePosition::EDGE_RIGHT) << endl;
        ...
        // cout << row << (boardSize[row] - 1) << VERTEX_BOTTOM_RIGHT;
        cout << *board[row][boardSize[row]-1]->getVertex(VertexPosition::VERTEX_BOTTOM_RIGHT);
        cout << endl;

        // Print the bottom slashes and spaces between hexagons
        if (row == 4){
            cout << setw(34 -  padding[row]) << " "; // Adjust leading spaces
            for (size_t col = 0; col < boardSize[row]; ++col) {
                cout << *board[row][col]->getEdge(EdgePosition::EDGE_BOTTOM_LEFT);
                cout << setw(5) << *board[row][col]->getEdge(EdgePosition::EDGE_BOTTOM_RIGHT);
                if (col < boardSize[row]) {
                    cout << setw(7);
                }
            }
    ...
    cout << setw(38 - padding[4]); // Adjust leading spaces
    for (size_t col = 0; col < boardSize[4]; ++col) {
        // cout << numberOfRows-1 << col << VERTEX_BOTTOM;
