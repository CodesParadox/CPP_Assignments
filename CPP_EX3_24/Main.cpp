...
int main()
{
    Player p1("LootGoblin");   //he's a thief
    Player p2("NoobMaster");  //he's a bully
    Player p3("Nagato");      //he's a god
...
void dealWithTrade(Player& currPlayer, Catan& catan){
    ...
    do {
        ...
        sendingResources.push_back(make_tuple((ResourceType)choiceSend, amountSend));
        chooseAnother = -1;
        if (choiceSend != 0){
    ...
    do {
        ...
        receivingResources.push_back(make_tuple((ResourceType)choiceReceive, amountReceive));
        chooseAnother = -1;
        if (choiceReceive != 0){
